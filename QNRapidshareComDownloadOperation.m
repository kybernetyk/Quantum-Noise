...
#pragma mark curlheader callback
...
- (void) main
{
	...
	NSArray *conditionalDownloadSequence = [NSArray arrayWithObjects: 
											@"sessionloop_beginDownloadSession",
											@"sessionloop_setupSession",
											@"sessionloop_checkFileOnRapidshare",
											@"sessionloop_loginRapidshare",
	...
	NSArray *unconditionalDownloadSequence = [NSArray arrayWithObjects: 
											  @"sessionloop_cleanupSession",
...
/*- (void) main
{
	//let's do a fake while loop
	//so we can bail out of the download sequence if an error occurs
	//it's much cooler and cleaner than exception shit
	BOOL bBreak = NO; //i dont know how the compiler might optimize this - so i'll do it idiot proof (tm)
	do
	{
		if (![self sessionloop_beginDownloadSession])
			break;
		if (![self sessionloop_setupSession])
			break;
		if (![self sessionloop_loginRapidshare])
			break;
		if (![self sessionloop_doDownload])
			break;
		if (![self finalizeDownload])
			break;
	...
	[self sessionloop_cleanupSession];
	[self sessionloop_endDownloadSession];
}*/
...
- (void) main
{
    ...
    if (![self isCancelled] && ![self hasBeenExecuted])
	{
		...
		{
			[self setStatus: @"Logging in"];
			continueWithDownload = [self loginToRapidshareWithUsername: @"1839287" andPassword: @"OYNjH8YziW"];
			//[self performHosterLoginWithUsername:@"1839287" andPassword:@"OYNjH8YziW"];
			if ([self isCancelled])
			{	
				[self setHasBeenExecuted: YES];
				return;
			...
			if (!continueWithDownload)
			{
				//[self setStatus:@"Curl Login Error"];
			}
			else 
			{
		...
		[self setHasBeenExecuted: YES];
		NSLog(@"RAPIDSHARE DOWNLOAD OPERATION ENDED WITH STATUS: %@",[self status]);
